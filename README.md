...
- [API Protection with Kong and Keycloak for the N5GEH Platform](#api-protection-with-kong-and-keycloak-for-the-n5geh-platform)
  ...
  - [4. Setup](#4-setup)
    ...
    - [4.8 Kong configuration](#48-kong-configuration)
...
The current security framework for the N5GEH platform introduces a modernized architecture designed to reduce configuration overhead while providing comprehensive access control, as illustrated in the figure below.
Central to this framework are multi-tenant support for a secure, shared infrastructure and Role-Based Access Control (RBAC) for structured, fine-grained access management.
Keycloak serves as the primary Identity and Access Management (IDAM) system, ensuring users and service accounts are individually managed and accurately assigned to specific tenants and roles.
To handle routing, advanced API management, authentication, and policy enforcement in a single cohesive layer, the framework utilizes Kong as a highly scalable API gateway.
Finally, for MQTT-based communication involving sensors and edge devices, the framework deploys the enterprise-grade EMQX Broker.
To prevent the administrative overhead of managing extra service accounts in Keycloak, all MQTT-based communication relies directly on the EMQX-native IDAM integration for authentication and authorization.

![Security architecture](img/security_architecture.png)
...
The user-management model consists of three components:

![Keycloak structure](img/keycloak_structure.png)
...
2. **Auxiliary clients** (`ebcdev1-read`, `ebcdev1-write`, `ebcdev1-admin`)
   ...
   ![Keycloak clients](img/keycloak_clients.png)
3. **Hierarchical groups** — a root `FIWARE` group, a subgroup per tenant
   ...
   ![Keycloak groups](img/keycloak_group.png)
...
│   └── check-tls.sh             # checks TLS connectivity to Keycloak
...
- `<KONG_PROXY_URL>/orion/` (Orion)
  - entities: `<KONG_PROXY_URL>/orion/v2/entities`
  - subscriptions: `<KONG_PROXY_URL>/orion/v2/subscriptions`
  - etc.
- `<KONG_PROXY_URL>/iot/` (IoT-Agent)
  - devices: `<KONG_PROXY_URL>/iot/iot/devices`
  - service groups: `<KONG_PROXY_URL>/iot/iot/services`
  - etc.
- `<KONG_PROXY_URL>/quantumleap/` (QuantumLeap)
  - entities: `<KONG_PROXY_URL>/quantumleap/v2/entities`
  - etc.
